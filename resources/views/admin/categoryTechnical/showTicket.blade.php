...
<div class="all-ticket" style="padding-top: 40px;padding-bottom: 40px">
    ...
    <a style="margin-bottom: 10px" href="{{route('technical.ticket.attachment.download',['attach'=>$attachment])}}" class="btn btn-default">{{$attachment}}</a>
    <br>
    @endforeach
    @endif
    ...
    <form action="{{route('technical.ticket.comment.store')}}" method="POST" enctype="multipart/form-data">
        @csrf
        <textarea name="message" class="form-control" id="description" style="height: 150px; box-shadow: 0px 0px 6px -2px rgb(39, 39, 39);"></textarea>
        <br>
        <div class="form-group" style="position: relative;">
            <div class="icon"style="position:absolute;top:2px">
            <i class="fa fa-paperclip"></i> <span style="color: rgb(95, 95, 95);">{{__('trans_word.Add attachment')}}</span>
            </div>
            <input style="opacity: 0;width: 114px;height: 21px;" type="file" name="attachment[]" multiple>
        </div>
